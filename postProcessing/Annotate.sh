...
for path in ${out}/*; do 
    ...
    if ! [ -z "$path2normal" ]; then
        ...
        echo "#!/bin/bash" >> Annotate_SUBMIT.sh
	...
	rm -f Annotate_SUBMIT.sh
        echo "#!/bin/bash" >> Annotate_SUBMIT.sh
	echo -e "sbatch ${path2SNVCurate}/runAnnotate.sh $path $out $path2Mutect $reference False $csv $path2SNVCurate" >> Annotate_SUBMIT.sh
    fi
done
